package cn.greentracks.keycloak.phone.authentication.authenticators.browser;

import cn.greentracks.keycloak.phone.authentication.forms.SupportPhonePages;
import cn.greentracks.keycloak.phone.providers.constants.TokenCodeType;
import cn.greentracks.keycloak.phone.providers.exception.PhoneNumberInvalidException;
import cn.greentracks.keycloak.phone.providers.spi.PhoneVerificationCodeProvider;
import cn.greentracks.common.OptionalUtils;
import cn.greentracks.keycloak.phone.Utils;
...
public class PhoneUsernamePasswordForm extends UsernamePasswordForm implements Authenticator, AuthenticatorFactory {
  ...
  private LoginFormsProvider assemblyForm(AuthenticationFlowContext context, LoginFormsProvider form){
    if (isSupportPhone(context))
      form.setAttribute(SupportPhonePages.ATTRIBUTE_SUPPORT_PHONE, true);
  ...
  protected boolean validateForm(AuthenticationFlowContext context, MultivaluedMap<String, String> inputData) {
    ...
    if (Validation.isBlank(phoneNumber)) {
      ...
      context.form().setAttribute(SupportPhonePages.ATTEMPTED_PHONE_ACTIVATED, true);
      assemblyForm(context,context.form());
      Response challengeResponse = challenge(context, SupportPhonePages.Errors.MISSING.message(), SupportPhonePages.FIELD_PHONE_NUMBER);
      context.forceChallenge(challengeResponse);
      return false;
    }

    String code = inputData.getFirst(SupportPhonePages.FIELD_VERIFICATION_CODE);
  ...
  private void invalidVerificationCode(AuthenticationFlowContext context, String number) {
    ...
    context.form().setAttribute(SupportPhonePages.ATTEMPTED_PHONE_ACTIVATED, true)
        .setAttribute(SupportPhonePages.ATTEMPTED_PHONE_NUMBER, number);
    assemblyForm(context,context.form());
    Response challengeResponse = challenge(context, SupportPhonePages.Errors.NOT_MATCH.message(), SupportPhonePages.FIELD_VERIFICATION_CODE);
     ...
     var validPhoneNumber = Utils.canonicalizePhoneNumber(context.getSession(),phoneNumber);
      ...
      return Utils.findUserByPhone(context.getSession(), context.getRealm(), validPhoneNumber)
          ...
          .orElseGet(() -> {
            ...
            assemblyForm(context,context.form());
            Response challengeResponse = challenge(context, SupportPhonePages.Errors.USER_NOT_FOUND.message(), SupportPhonePages.FIELD_PHONE_NUMBER);
      ...
      context.form().setAttribute(SupportPhonePages.ATTEMPTED_PHONE_ACTIVATED, true)
          .setAttribute(SupportPhonePages.ATTEMPTED_PHONE_NUMBER, phoneNumber);
      assemblyForm(context,context.form());
      Response challengeResponse = challenge(context,e.getErrorType().message(), SupportPhonePages.FIELD_PHONE_NUMBER);
      context.failureChallenge(AuthenticationFlowError.INVALID_USER, challengeResponse);
      return false;
    }
  }
